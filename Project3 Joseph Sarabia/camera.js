function Camera(gl,d,modelUp) // Compute a camera from model's bounding box dimensions
{
	...
	this.truck=function(view, delta){
		...
		//addMessage(view[2]);
		//console.log(viewMatrix);
		/*view.translate(-view.elements[0] * delta, -view.elements[4] * delta, -view.elements[8]* delta);
		var newEye = new Vector3(view.multiplyVector3(new Vector3(eye)));
		//addMessage(eye[2] + "   " + newEye.elements[0]);
		eye[0] = newEye.elements[0];
		eye[1] = newEye.elements[1];
		eye[2] = newEye.elements[2];
		
		
		var newAt = new Vector3(view.multiplyVector3(new Vector3(at)));
	...
	/*	
		//addMessage(view[2]);
		//console.log(viewMatrix);
		view.translate(-view.elements[1] * delta, -view.elements[5] * delta, -view.elements[9]* delta);
		var newEye = new Vector3(view.multiplyVector3(new Vector3(eye)));
		addMessage(eye[2] + "   " + newEye[0]);
		eye[0] = newEye.elements[0];
		eye[1] = newEye.elements[1];
		eye[2] = newEye.elements[2];
	...
	*/	
	...
	this.pan=function(view, angle){
		...
		/*view.translate(eye[0],eye[1],eye[2]).rotate(angle, view.elements[1], view.elements[5], view.elements[9]).translate(-eye[0],-eye[1],-eye[2]);
